...
class ShippingOperationReport(models.Model):
    ...
    loading_port_id = fields.Many2one("freight.port", string="Loading Port", readonly=1)
    discharg_port_id = fields.Many2one(
        "freight.port", string="Discharging Port", readonly=1
    )
    direction = fields.Selection(
        [("import", "Import"), ("export", "Export")], string="Direction", readonly=1
    )
    ...
    order_date = fields.Datetime(string="Order Date", readonly=1)
    inv_payment = fields.Float(string="Invoice Amount", readonly=1)
    bill_payment = fields.Float(string="Bill Amount", readonly=1)
