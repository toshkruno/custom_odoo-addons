...
class ResCompany(models.Model):
    ...
    def _default_report_template(self):
        ...
        report_id = report_obj.search([('model', '=', 'account.move'), (
            'report_name', '=', 'tritel_report_template.report_invoice_template_custom')])
        if report_id:
    ...
    def _get_default_image(self, is_company, colorize=False):
        img_path = get_module_resource(
            'tritel_report_template', 'static/src/img', 'avatar.png')
    ...
    def action_view_report_extra_content(self):
        action = self.env.ref(
            'tritel_report_template.action_report_extracontent').read()[0]
